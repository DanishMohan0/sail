...
class controlConfigurationService(Node):
    ...
    def sail_pub_callback(self):
        ...
        #Possible thrust vectors w.r.t. compass
        t1 = (self.windAngle + angle_of_attack + thrust_sail_offset) % 360
        t2 = (self.windAngle - angle_of_attack - thrust_sail_offset) % 360

        #Decide which "tack" is closer to target heading
        if abs(shortestAngle(t1, self.targetHeading)) <= abs(shortestAngle(t2, self.targetHeading)): #bias
            optimal_compass_angle = (self.windAngle + angle_of_attack) % 360
        else:
            optimal_compass_angle = (self.windAngle - angle_of_attack) % 360

        #Publish target angle relative to boat bow
        msg.data = float(shortestAngle(self.heading, optimal_compass_angle))
